...
from app.kb.policy import query_policy
...
- policy_ref：引用的政策条款原文
- confidence：置信度

注意：policy_ref 必须来自提供的政策条款原文，不要编造。
"""
...
def aftersale_node(state: CustomerServiceState) -> dict:
    """售后核实：查政策 + 查订单 → 结构化输出 AftersaleResult"""
    user_msg = state["messages"][-1].content

    # 1. 查政策（换货条件）和订单信息
    replace_condition = query_policy("replace_condition")
    order_info = query_order(DEMO_ORDER_NO)

    # 2. 结构化输出 AftersaleResult
    try:
